...
def check_whether_balanced(expression, options):
    ...
    #  Parse.
    parsed_ce = _ce_parser.parse(expression, _ce_token.tokenize(expression, options), options)
    ...
    pfx_list = []
    for ce_item in parsed_ce.get_left_items():
        pfx_list.append(ce_item.get_origin_prefix())
    for ce_item in parsed_ce.get_right_items():
        pfx_list.append(ce_item.get_origin_prefix())
